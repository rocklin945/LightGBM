...
SCALER_PATH = f'{MODEL_DIR}/{MODEL_NAME}/scaler.pkl'
...
def load_and_split_data():
    ...
    if not os.path.exists(LABELED_DATA_PATH):
        raise FileNotFoundError(f"未找到标注数据文件: {LABELED_DATA_PATH}")
...
def train_model(X_train, y_train):
    """训练模型"""
    print("\n" + "=" * 80)
    print("2. 训练 Logistic Regression 模型")
    print("=" * 80)
...
def visualize_results(model, X_test, y_test, eval_results):
    ...
    colors = ['green' if c > 0 else 'red' for c in feature_df['coefficient'].values]
    axes[0, 0].barh(y_pos, feature_df['abs_coefficient'].values, color=colors, edgecolor='black', alpha=0.7)
...
def main():
    ...
    try:
        ...
        eval_results = evaluate_model(model, scaler, X_test, y_test)
